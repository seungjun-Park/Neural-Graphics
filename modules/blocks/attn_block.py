...
class AttnBlock(nn.Module):
    ...
    def forward(self, x, cond=None) -> torch.Tensor:
        ...
        if cond is not None:
            ...
            cond = h
        h, _ = self.self_attn(h, cond, cond, need_weights=False)
        h = self.drop(h)
        h = h + x
...
class ComplexAttnBlock(nn.Module):
    def __init__(self,
        ...
        self.ln = ComplexLayerNorm(in_channels)
        self.drop = ComplexDropout(dropout)

        self.self_amp_attn = nn.MultiheadAttention(in_channels,
                                                   num_heads=self.heads,
                                                   dropout=attn_dropout,
                                                   batch_first=True,
                                                   bias=bias)

        self.self_phase_attn = nn.MultiheadAttention(in_channels,
                                                     num_heads=self.heads,
                                                     dropout=attn_dropout,
                                                     batch_first=True,
                                                     bias=bias)

        self.phase_amp_attn = nn.MultiheadAttention(in_channels,
                                                    num_heads=self.heads,
                                                    dropout=attn_dropout,
                                                    batch_first=True,
                                                    bias=bias)

        self.mlp = ComplexMLP(in_channels * 3, int(in_channels * 3 * mlp_ratio), dropout=dropout, act=act)
        self.ln2 = nn.LayerNorm(in_channels)
