...
at::Tensor _deform_conv_nd_forward_cuda(
	...
	AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "deform_conv_nd_forward<>", [&]() {
		...
		if (bias.defined())
		{
			// reshape output to [batch_size, output_sizes, out_channels]
			output = output.reshape({ batch_size, out_channels, -1 }).transpose(1, 2);
			// bias shape == [out_channels]
			output += bias;
			// reshape output to output_size
...
std::vector<at::Tensor> _deform_conv_nd_backward_cuda(
	...
	AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "get_blocks", [&]() {
		using scalar_t = scalar_t;
		int min_grid_size_im2col, block_size_im2col;
		int min_grid_size_col2im, block_size_col2im;
